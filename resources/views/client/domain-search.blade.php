...
<div style="max-width:900px;margin:0 auto;padding:24px 16px;">
    ...
    <form method="GET" action="{{ route(client.domain.search) }}" id="domain-search-form" style="margin-bottom:32px;">
        <div style="display:flex;background:#fff;border:2px solid #e2e8f0;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);transition:border-color 0.2s;" onfocusin="this.style.borderColor=#1a4d80" onfocusout="this.style.borderColor=#e2e8f0">
            <input
                type="text"
                name="domain"
                id="domain-input"
                value="{{ $searchDomain ? preg_replace(/.[^.]+$/, , $searchDomain) : old(domain) }}"
            ...
            <div style="display:flex;align-items:center;border-left:1px solid #e2e8f0;">
                <select name="tld" id="tld-select" style="border:none;outline:none;font-size:16px;font-weight:600;color:#1a4d80;background:transparent;padding:0 12px;cursor:pointer;font-family:inherit;-webkit-appearance:auto;appearance:auto;height:58px;">
            ...
            <button type="submit" style="padding:0 32px;background:#1a4d80;color:#fff;font-size:16px;font-weight:700;border:none;cursor:pointer;transition:background 0.2s;font-family:inherit;white-space:nowrap;" onmouseover="this.style.background=#163d66" onmouseout="this.style.background=#1a4d80">
                Search
            </button>
        </div>
    </form>
    ...
    <div style="background:#fff;border-radius:12px;border:2px solid {{ $primary[available] ? #22c55e : #ef4444 }};padding:20px 24px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
        ...
        <div style="display:flex;align-items:center;gap:20px;">
            ...
            <form method="POST" action="{{ route(client.cart.add-domain) }}" style="display:flex;gap:8px;">
                @csrf
                <input type="hidden" name="domain" value="{{ $primary[domain] }}">
                <input type="hidden" name="type" value="register">
                <input type="hidden" name="years" value="1">
                <button type="submit" style="padding:12px 24px;background:#06d6a0;color:#0f172a;font-weight:700;font-size:15px;border:none;border-radius:8px;cursor:pointer;font-family:inherit;transition:background 0.2s;" onmouseover="this.style.background=#05c493" onmouseout="this.style.background=#06d6a0">
                    Add to Cart
                </button>
            </form>
            @else
            <form method="POST" action="{{ route(client.cart.add-domain) }}" style="display:flex;gap:8px;">
                ...
                <input type="hidden" name="years" value="1">
                <button type="submit" style="padding:12px 24px;background:#64748b;color:#fff;font-weight:700;font-size:15px;border:none;border-radius:8px;cursor:pointer;font-family:inherit;" onmouseover="this.style.background=#475569" onmouseout="this.style.background=#64748b">
                    Transfer Domain
                </button>
            </form>
            @endif
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc;">
                    <th style="padding:10px 16px;text-align:left;font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Extension</th>
                    <th style="padding:10px 16px;text-align:center;font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Register</th>
                    <th style="padding:10px 16px;text-align:center;font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Transfer</th>
                    <th style="padding:10px 16px;text-align:center;font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Renew</th>
                    <th style="padding:10px 16px;text-align:center;font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Action</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background=#f8fafc" onmouseout="this.style.background=transparent">
                    ...
                    <td style="padding:10px 16px;text-align:center;">
                        <button onclick="document.getElementById(domain-input).value=;document.getElementById(tld-select).value={{ - }};document.getElementById(domain-input).focus();" style="padding:5px 12px;background:#eff6ff;color:#1a4d80;font-size:12px;font-weight:600;border:1px solid #bfdbfe;border-radius:5px;cursor:pointer;">Search</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
