...
<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:16px;margin-bottom:24px;">
    ...
    <div class="stat-card" style="border-left:4px solid #008b8b;">
        ...
        <div class="stat-label">{{ __('admin.dashboard.active_domains') }}</div>
    </div>
    ...
    <div class="stat-card" style="border-left:4px solid #d68100;">
        ...
        <div class="stat-label">{{ __('admin.dashboard.unpaid_invoices') }}</div>
    </div>
...
<div class="card" style="margin-bottom:24px;">
    <div class="card-header">
        {{ __('admin.dashboard.income_overview') }}
    </div>
    <div class="card-body" style="display:grid;grid-template-columns:repeat(3,1fr);gap:0;">
        <div style="text-align:center;padding:16px;border-right:1px solid #e5e7eb;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:#999;margin-bottom:6px;">{{ __('admin.dashboard.this_month') }}</div>
            <div style="font-size:24px;font-weight:700;color:#46a546;">${{ number_format($monthIncome, 2) }}</div>
        </div>
        <div style="text-align:center;padding:16px;border-right:1px solid #e5e7eb;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:#999;margin-bottom:6px;">{{ __('admin.dashboard.this_week') }}</div>
            <div style="font-size:24px;font-weight:700;color:#337ab7;">${{ number_format($weekIncome, 2) }}</div>
        </div>
        <div style="text-align:center;padding:16px;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:#999;margin-bottom:6px;">{{ __('admin.dashboard.today') }}</div>
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
    ...
    <div class="card">
        <div class="card-header">
            {{ __('admin.dashboard.recent_clients') }}
            <a href="{{ route("admin.clients.index") }}" style="font-size:12px;font-weight:400;color:#337ab7;text-decoration:none;">{{ __('admin.dashboard.view_all') }}</a>
        </div>
        <div>
            ...
            @empty
            <p style="padding:16px;font-size:13px;color:#999;margin:0;">{{ __('admin.dashboard.no_clients') }}</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            {{ __('admin.dashboard.recent_tickets') }}
            <a href="{{ route("admin.tickets.index") }}" style="font-size:12px;font-weight:400;color:#337ab7;text-decoration:none;">{{ __('admin.dashboard.view_all') }}</a>
        </div>
        <div>
            ...
            @empty
            <p style="padding:16px;font-size:13px;color:#999;margin:0;">{{ __('admin.dashboard.no_tickets') }}</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            {{ __('admin.dashboard.recent_orders') }}
            <a href="{{ route("admin.orders.index") }}" style="font-size:12px;font-weight:400;color:#337ab7;text-decoration:none;">{{ __('admin.dashboard.view_all') }}</a>
        </div>
        <div>
            ...
            @empty
            <p style="padding:16px;font-size:13px;color:#999;margin:0;">{{ __('admin.dashboard.no_orders') }}</p>
            @endforelse
        </div>
    </div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
    ...
    <!-- System Information -->
    <div class="card">
        <div class="card-header">{{ __('admin.dashboard.system_information') }}</div>
        <div class="card-body">
            <table style="width:100%;font-size:13px;border-collapse:collapse;">
                <tr style="border-bottom:1px solid #f0f0f0;">
                    <td style="padding:6px 0;color:#666;width:50%;">{{ __('admin.dashboard.pnlcs_version') }}</td>
                    <td style="padding:6px 0;font-weight:500;">1.0.0</td>
                </tr>
                <tr style="border-bottom:1px solid #f0f0f0;">
                    <td style="padding:6px 0;color:#666;">{{ __('admin.dashboard.laravel') }}</td>
                    <td style="padding:6px 0;font-weight:500;">{{ app()->version() }}</td>
                </tr>
                <tr style="border-bottom:1px solid #f0f0f0;">
                    <td style="padding:6px 0;color:#666;">{{ __('admin.dashboard.php') }}</td>
                    <td style="padding:6px 0;font-weight:500;">{{ phpversion() }}</td>
                </tr>
                <tr style="border-bottom:1px solid #f0f0f0;">
                    <td style="padding:6px 0;color:#666;">{{ __('admin.dashboard.database') }}</td>
                    <td style="padding:6px 0;font-weight:500;">MySQL {{ DB::selectOne("SELECT VERSION() as v")->v }}</td>
                </tr>
                <tr>
                    <td style="padding:6px 0;color:#666;">{{ __('admin.dashboard.server') }}</td>
    ...
    <!-- Quick Actions -->
    <div class="card">
        <div class="card-header">{{ __('admin.dashboard.quick_actions') }}</div>
        <div class="card-body">
