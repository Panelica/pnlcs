    ...
    <div class="card" style="margin-bottom:15px;">
        ...
        <div class="card-body">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                ...
                <div class="form-group" x-data="{ show: '{{ $product->type }}' === 'ssl' }" x-show="show" x-cloak><label class="form-label">{{ __('admin.products.ssl_module') }}</label><select name="ssl_module" class="form-control"><option value="">{{ __('admin.products.ssl_none') }}</option>@foreach($sslModules + array_filter([(string) $product->ssl_module => (string) $product->ssl_module]) as $sslKey => $sslLabel)<option value="{{ $sslKey }}" @selected($product->ssl_module === $sslKey)>{{ $sslLabel }}</option>@endforeach</select></div>
