    ...
    <div style="grid-column:span 2;">
        ...
        <div class="card" style="margin-bottom:15px;">
            ...
            <table class="data-table">
                ...
                <tfoot>
                    <tr><td colspan="2" style="text-align:right;padding:8px 12px;color:#555;">{{ __('admin.invoices.subtotal') }}</td><td style="text-align:right;padding:8px 12px;font-weight:600;font-family:monospace;">${{ number_format($invoice->subtotal, 2) }}</td></tr>
                    @if($invoice->tax > 0)
                    <tr><td colspan="2" style="text-align:right;padding:4px 12px;color:#555;">{{ __('admin.invoices.tax') }}{{ $invoice->tax_rate > 0 ? " (" . $invoice->tax_rate . "%)" : "" }}</td><td style="text-align:right;padding:4px 12px;font-family:monospace;">${{ number_format($invoice->tax, 2) }}</td></tr>
                    @endif
                    @if($invoice->tax2 > 0)
                    <tr><td colspan="2" style="text-align:right;padding:4px 12px;color:#555;">{{ __('admin.invoices.tax_2') }}{{ $invoice->tax_rate2 > 0 ? " (" . $invoice->tax_rate2 . "%)" : "" }}</td><td style="text-align:right;padding:4px 12px;font-family:monospace;">${{ number_format($invoice->tax2, 2) }}</td></tr>
                    @endif
                    @if($invoice->credit > 0)
                    <tr><td colspan="2" style="text-align:right;padding:4px 12px;color:#5cb85c;">{{ __('admin.invoices.credit_applied') }}</td><td style="text-align:right;padding:4px 12px;font-family:monospace;color:#5cb85c;">-${{ number_format($invoice->credit, 2) }}</td></tr>
                    @endif
                    <tr style="border-top:2px solid #aaa;background:#f5f5f5;"><td colspan="2" style="text-align:right;padding:8px 12px;font-weight:700;font-size:14px;">{{ __('admin.invoices.total') }}</td><td style="text-align:right;padding:8px 12px;font-weight:700;font-size:14px;font-family:monospace;">${{ number_format($invoice->total, 2) }}</td></tr>
                </tfoot>
            </table>
        </div>
