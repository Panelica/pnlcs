...
<div style="display:grid;grid-template-columns:1fr 260px;gap:15px;">
    <div>
        <div class="card" style="margin-bottom:15px;">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="5" style="text-align:center;color:#999;padding:20px;">No line items.</td></tr>
                    @endforelse
                </tbody>
                <tfoot style="border-top:2px solid #aaa;">
                    <tr><td colspan="4" style="padding:8px 12px;text-align:right;font-weight:600;">Subtotal</td><td style="padding:8px 12px;text-align:right;font-weight:600;font-family:monospace;">${{ number_format($quote->subtotal,2) }}</td></tr>
                    @if($quote->tax>0)<tr><td colspan="4" style="padding:4px 12px;text-align:right;color:#777;">Tax</td><td style="padding:4px 12px;text-align:right;font-family:monospace;">${{ number_format($quote->tax,2) }}</td></tr>@endif
                    <tr style="background:#f5f5f5;"><td colspan="4" style="padding:8px 12px;text-align:right;font-weight:700;font-size:14px;">Total</td><td style="padding:8px 12px;text-align:right;font-weight:700;font-size:14px;font-family:monospace;">${{ number_format($quote->total,2) }}</td></tr>
                </tfoot>
            </table>
        </div>
        ...
        @if($quote->proposal)
        <div class="card" style="margin-bottom:15px;">
            <div class="card-header"><strong>Proposal</strong></div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div style="margin-bottom:10px;">
                    <p style="font-size:11px;font-weight:600;color:#777;text-transform:uppercase;margin-bottom:4px;">Admin Notes</p>
                    <p style="font-size:13px;color:#555;white-space:pre-wrap;">{{ $quote->notes }}</p>
                </div>
                @endif
                @if($quote->customer_notes)
                <div>
                    <p style="font-size:11px;font-weight:600;color:#777;text-transform:uppercase;margin-bottom:4px;">Customer Notes</p>
    ...
    <div>
        ...
        <div class="panel">
            ...
            <div class="panel-body" style="display:flex;flex-direction:column;gap:6px;">
                <a href="{{ route('admin.quotes.edit', $quote) }}" class="btn btn-default btn-sm" style="width:100%;text-align:center;">Edit Quote</a>
                @if($quote->status === 'Draft')
                <form method="POST" action="{{ route('admin.quotes.send', $quote) }}">
                    @csrf<button type="submit" class="btn btn-primary btn-sm" style="width:100%;">Send to Client</button>
                </form>
                @endif
                @if($quote->status === 'Sent')
                <form method="POST" action="{{ route('admin.quotes.accept', $quote) }}">
                    @csrf<button type="submit" class="btn btn-success btn-sm" style="width:100%;">Accept Quote</button>
                </form>
                <form method="POST" action="{{ route('admin.quotes.decline', $quote) }}">
                    @csrf<button type="submit" class="btn btn-danger btn-sm" style="width:100%;">Decline Quote</button>
                </form>
                @endif
                @if($quote->status === 'Accepted')
                <form method="POST" action="{{ route('admin.quotes.convert', $quote) }}">
                    @csrf<button type="submit" class="btn btn-primary btn-sm" style="width:100%;">Convert to Invoice</button>
                </form>
                @endif
                <form method="POST" action="{{ route('admin.quotes.destroy', $quote) }}" onsubmit="return confirm('Delete this quote?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="btn btn-default btn-sm" style="width:100%;color:#d9534f;">Delete Quote</button>
                </form>
            </div>
        </div>
    </div>
