    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:420px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        ...
        <form method="POST" action="{{ route('admin.config.server-groups.store') }}">
            ...
            <div style="padding:20px;">
                ...
                <div class="form-group"><label class="form-label">{{ __('admin.server_groups.fill_type') }}</label><select name="fill_type" class="form-control"><option value="sequential">{{ __('admin.server_groups.sequential') }}</option><option value="roundrobin">{{ __('admin.server_groups.roundrobin') }}</option></select></div>
            </div>
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:420px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        ...
        <form method="POST" id="edit-sg-form" action="">
            ...
            <div style="padding:20px;">
                ...
                <div class="form-group"><label class="form-label">{{ __('admin.server_groups.fill_type') }}</label><select name="fill_type" id="esg-fill" class="form-control"><option value="sequential">{{ __('admin.server_groups.sequential_short') }}</option><option value="roundrobin">{{ __('admin.server_groups.roundrobin_short') }}</option></select></div>
            </div>
            <div style="padding:12px 20px;border-top:1px solid #e5e5e5;display:flex;gap:8px;justify-content:flex-end;">
