...
<p style="color:#666;font-size:13px;margin-bottom:15px;">API credentials allow external applications to interact with PNLCS via the WHMCS-compatible API. Each key has a unique identifier and secret. <a href="{{ route(admin.api-docs) }}" style="color:#337ab7;">View API Documentation</a></p>
...
<div style="padding:15px;background:#fcf8e3;border:1px solid #faebcc;border-radius:4px;margin-bottom:15px;">
    <strong style="color:#8a6d3b;">API Secret (save this now — it will never be shown again!):</strong>
    <code style="display:block;margin-top:8px;padding:10px;background:#fff;border:1px solid #ddd;border-radius:3px;font-size:14px;word-break:break-all;">{{ session(new_secret) }}</code>
    <p style="margin:8px 0 0;font-size:12px;color:#a94442;">Store this secret securely. It cannot be retrieved after you leave this page.</p>
</div>
@endif
...
<div style="margin-top:15px;padding:12px 15px;background:#fcf8e3;border:1px solid #faebcc;border-radius:4px;color:#8a6d3b;font-size:13px;">
    <strong>API Usage:</strong> Send POST requests to <code>/api/v1/{action}</code> with parameters <code>identifier</code> and <code>secret</code> for authentication. <a href="{{ route(admin.api-docs) }}" style="color:#337ab7;">View Full API Documentation</a>
</div>
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:450px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        ...
        <form method="POST" action="{{ route(admin.config.api-credentials.store) }}">
            ...
            <div style="padding:20px;">
                ...
                <p style="font-size:12px;color:#999;margin-top:8px;">A unique API Identifier and Secret will be auto-generated. <strong>Save the secret immediately</strong> — it cannot be retrieved later.</p>
            </div>
