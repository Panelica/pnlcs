...
<div class="card">
    ...
    <table class="data-table">
        ...
        <tr>
            ...
            <td style="font-size:12px;">{{ $ban->created_at->format('d M Y H:i') }}</td>
            <td style="font-size:12px;">{{ $ban->expires_at?->format('d M Y H:i') ?? 'Never' }}</td>
            <td style="text-align:right;">
                <form method="POST" action="{{ route('admin.config.banned-ips.destroy', $ban) }}" style="display:inline;" onsubmit="return confirm('Unban IP {{ $ban->ip }}?')">
                    @csrf @method('DELETE')
...
<div id="modal-add-ip" style="display:none;position:fixed;inset:0;z-index:1050;align-items:center;justify-content:center;">
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:420px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        <div style="padding:15px 20px;border-bottom:1px solid #e5e5e5;display:flex;align-items:center;justify-content:space-between;">
            <h4 style="margin:0;font-size:16px;">Ban IP Address</h4>
        ...
        <form method="POST" action="{{ route('admin.config.banned-ips.store') }}">
            ...
            <div style="padding:20px;">
                <div class="form-group"><label class="form-label">IP Address</label><input type="text" name="ip" required class="form-control" placeholder="192.168.1.1"></div>
