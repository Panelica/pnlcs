...
<div class="page-header" style="display:flex;align-items:center;justify-content:space-between;">
    <h1>{{ __('admin.notifications.title') }}</h1>
    <button type="button" onclick="document.getElementById('modal-add-provider').style.display='flex'" class="btn btn-primary btn-sm">+ {{ __('admin.notifications.add_provider') }}</button>
</div>

{{-- Providers --}}
<div class="card">
    <div class="card-header" style="padding:12px 20px;border-bottom:1px solid #e5e5e5;font-weight:600;">
        <i class="fas fa-bell"></i> {{ __('admin.notifications.providers') }}
    </div>
    @if($providers->isEmpty())
        <div class="card-body" style="text-align:center;padding:40px;color:#999;">{{ __('admin.notifications.no_providers') }}</div>
    @else
    <table class="data-table">
        <thead><tr>
            ...
            <th>{{ __('admin.notifications.rules') }}</th>
        ...
        <tr>
            ...
            <td>
                @if($provider->active)
                    <span class="badge badge-active">{{ __('common.status.active') }}</span>
                @else
                    <span class="badge badge-open">{{ __('common.status.inactive') }}</span>
                @endif
            </td>
...
<div style="margin-top:20px;">
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;">
        <h2 style="font-size:18px;">{{ __('admin.notifications.notification_rules') }}</h2>
        @if($providers->isNotEmpty())
        <button type="button" onclick="document.getElementById('modal-add-rule').style.display='flex'" class="btn btn-primary btn-sm">+ {{ __('admin.notifications.add_rule') }}</button>
        @endif
    </div>
    <div class="card">
        @php
        ...
        @endphp
        @if($allRules->isEmpty())
            <div class="card-body" style="text-align:center;padding:40px;color:#999;">{{ __('admin.notifications.no_rules') }}</div>
        @else
        <table class="data-table">
            <thead><tr>
                <th>{{ __('admin.notifications.event') }}</th>
                <th>{{ __('admin.notifications.provider') }}</th>
                <th>{{ __('common.table.status') }}</th>
                <th style="text-align:right;">{{ __('common.table.actions') }}</th>
            </tr></thead>
            <tbody>
...
<div id="modal-add-provider" style="display:none;position:fixed;inset:0;z-index:1050;align-items:center;justify-content:center;">
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:500px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        <div style="padding:15px 20px;border-bottom:1px solid #e5e5e5;display:flex;align-items:center;justify-content:space-between;">
            <h4 style="margin:0;font-size:16px;">{{ __('admin.notifications.add_provider') }}</h4>
        ...
        <form method="POST" action="{{ route('admin.config.notification-providers.store') }}">
            ...
            <div style="padding:20px;">
                ...
                <div id="add-slack-fields" style="display:none;">
                    <div class="form-group"><label class="form-label">{{ __('admin.notifications.slack_webhook_url') }}</label><input type="url" name="settings[webhook_url]" class="form-control" placeholder="https://hooks.slack.com/services/..."></div>
                    <div class="form-group"><label class="form-label">{{ __('admin.notifications.bot_username') }}</label><input type="text" name="settings[username]" class="form-control" placeholder="PNLCS" value="PNLCS"></div>
                </div>
            ...
            <div style="padding:12px 20px;border-top:1px solid #e5e5e5;display:flex;gap:8px;justify-content:flex-end;">
                ...
                <button type="submit" class="btn btn-primary btn-sm">{{ __('admin.notifications.create_provider') }}</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-edit-provider" style="display:none;position:fixed;inset:0;z-index:1050;align-items:center;justify-content:center;">
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:500px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        <div style="padding:15px 20px;border-bottom:1px solid #e5e5e5;display:flex;align-items:center;justify-content:space-between;">
            <h4 style="margin:0;font-size:16px;">{{ __('admin.notifications.edit_provider') }}</h4>
        ...
        <form id="edit-provider-form" method="POST" action="">
            ...
            <div style="padding:20px;">
                ...
                <div id="edit-slack-fields" style="display:none;">
                    <div class="form-group"><label class="form-label">{{ __('admin.notifications.slack_webhook_url') }}</label><input type="url" name="settings[webhook_url]" id="edit-slack-url" class="form-control"></div>
                    <div class="form-group"><label class="form-label">{{ __('admin.notifications.bot_username') }}</label><input type="text" name="settings[username]" id="edit-slack-user" class="form-control"></div>
                </div>
...
<div id="modal-add-rule" style="display:none;position:fixed;inset:0;z-index:1050;align-items:center;justify-content:center;">
    ...
    <div style="position:relative;background:#fff;border-radius:4px;width:480px;max-width:95%;box-shadow:0 5px 30px rgba(0,0,0,0.3);">
        <div style="padding:15px 20px;border-bottom:1px solid #e5e5e5;display:flex;align-items:center;justify-content:space-between;">
            <h4 style="margin:0;font-size:16px;">{{ __('admin.notifications.add_rule') }}</h4>
        ...
        <form method="POST" action="{{ route('admin.config.notification-rules.store') }}">
            @csrf
            <div style="padding:20px;">
                <div class="form-group"><label class="form-label">{{ __('admin.notifications.provider') }} *</label>
                ...
                <div class="form-group"><label class="form-label">{{ __('admin.notifications.event_type') }} *</label>
                ...
                <div class="form-group"><label class="form-label">{{ __('admin.notifications.recipient_email') }}</label>
            ...
            <div style="padding:12px 20px;border-top:1px solid #e5e5e5;display:flex;gap:8px;justify-content:flex-end;">
                ...
                <button type="submit" class="btn btn-primary btn-sm">{{ __('admin.notifications.add_rule') }}</button>
            </div>
        </form>
    </div>
</div>
