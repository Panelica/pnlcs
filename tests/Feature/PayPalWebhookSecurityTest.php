 ...
 */
function paypalCapturePayer(Client $client): User
{
...
test('the paypal capture endpoint refuses a forged capture id and leaves the invoice unpaid', function () {
    ...
    $response = $this->actingAs(paypalCapturePayer($client))->postJson("/gateway/paypal/capture/{$invoice->id}", ['capture_id' => 'ATTACKER-CAP']);
...
test('the paypal capture endpoint credits only the verified amount, not the client total', function () {
    ...
    $response = $this->actingAs(paypalCapturePayer($client))->postJson("/gateway/paypal/capture/{$invoice->id}", ['capture_id' => 'GENUINE-CAP']);
