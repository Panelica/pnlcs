...
class OrdersWidget implements WidgetModuleInterface
{
    ...
    public function getData(): array
    {
        return [
            ...
            "recent" => DB::table("orders")->leftJoin("clients", "clients.id", "=", "orders.client_id")->select("orders.id", "orders.order_num", "orders.status", "orders.amount", "orders.created_at", DB::raw("CONCAT(clients.first_name, ' ', clients.last_name) as client"))->orderBy("orders.created_at", "desc")->limit(5)->get(),
        ];
    }
    ...
    public function render(array $data): string
    {
        ...
        foreach ($data["recent"] as $o) { $html .= '<div style="padding:8px 16px;border-bottom:1px solid var(--pn-border);font-size:13px;"><a href="/admin/orders/'.$o->id.'" style="color:var(--pn-link);">#'.$o->order_num.'</a> — '.$o->client.'<span style="float:right;">$'.number_format($o->amount,2).'</span></div>'; }
        return $html;
    }
}