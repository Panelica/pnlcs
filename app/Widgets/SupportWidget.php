...
class SupportWidget implements WidgetModuleInterface
{
    ...
    public function getData(): array
    {
        return [
            ...
            "assigned" => DB::table("tickets")->whereNotNull("admin_id")->where("status", "!=", "closed")->count(),
            "recent" => DB::table("tickets")->leftJoin("ticket_departments", "ticket_departments.id", "=", "tickets.department_id")->select("tickets.id", "tickets.subject", "tickets.status", "tickets.updated_at", "ticket_departments.name as dept")->orderBy("tickets.updated_at", "desc")->limit(5)->get(),
        ];
    }
    ...
    public function render(array $data): string
    {
        ...
        foreach ($data["recent"] as $t) { $html .= '<div style="padding:8px 14px;border-bottom:1px solid var(--pn-border);font-size:13px;"><a href="/admin/tickets/'.$t->id.'" style="color:var(--pn-link);">#'.$t->id.' '.$t->subject.'</a><span style="float:right;font-size:11px;color:var(--pn-muted);">'.\Carbon\Carbon::parse($t->updated_at)->diffForHumans().'</span></div>'; }
        return $html;
    }
}