...
class SupportWidget implements WidgetModuleInterface
{
    ...
    public function render(array $data): string
    {
        $html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0;border-bottom:1px solid var(--pn-border);">
            <div style="text-align:center;padding:14px;border-right:1px solid var(--pn-border);"><div style="font-size:24px;font-weight:700;color:#337ab7;">'.e($data["awaiting"]).'</div><div style="font-size:11px;color:var(--pn-muted);">'.e(__('admin.dashboard.awaiting_reply')).'</div></div>
            <div style="text-align:center;padding:14px;"><div style="font-size:24px;font-weight:700;color:#c43c35;">'.e($data["assigned"]).'</div><div style="font-size:11px;color:var(--pn-muted);">'.e(__('admin.dashboard.assigned')).'</div></div>
        </div>';
