...
class ToDoWidget implements WidgetModuleInterface
{
    ...
    public function render(array $data): string
    {
        ...
        $html = "";
        foreach ($data as $t) { $html .= '<div style="padding:8px 16px;border-bottom:1px solid var(--pn-border);font-size:13px;display:flex;align-items:center;gap:8px;"><span style="width:8px;height:8px;border-radius:50%;background:'.(($t["status"] ?? "")==="in-progress"?"#f89406":"#337ab7").'"></span>'.$t["title"].(($t["due_date"] ?? null) ? '<span style="margin-left:auto;font-size:11px;color:var(--pn-muted);">'.$t["due_date"].'</span>' : '').'</div>'; }
        return $html;
    }
}