...
class DomainsWidget implements WidgetModuleInterface
{
    ...
    public function getData(): array
    {
        return [
            ...
            "upcoming" => DB::table("domains")->select("domain", "expiry_date")->where("status", "active")->whereRaw("expiry_date BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY)")->orderBy("expiry_date")->limit(5)->get(),
        ];
    }
    ...
    public function render(array $data): string
    {
        ...
        if (empty($data["upcoming"]->toArray())) { $html .= '<div style="padding:16px;text-align:center;font-size:13px;color:var(--pn-muted);">No domains expiring soon</div>'; }
        return $html;
    }
}