...
class SslMonitoringReport extends AbstractReport
{
    ...
    public function generate(Request $request): array
    {
        $rows = DB::table("ssl_orders")
            ...
            ->selectRaw("ssl_orders.id, CONCAT(clients.first_name, ' ', clients.last_name) as client, ssl_orders.domain, ssl_orders.status, ssl_orders.expires_at, CASE WHEN ssl_orders.expires_at IS NOT NULL THEN DATEDIFF(ssl_orders.expires_at, NOW()) ELSE NULL END as days_left")
            ->orderByRaw("CASE WHEN ssl_orders.expires_at IS NULL THEN 1 ELSE 0 END, ssl_orders.expires_at")->get();
